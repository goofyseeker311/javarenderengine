		...
		if ((vsurf!=null)&&(campos!=null)) {
			...
			for (int i=0;i<vsurf.length;i++) {
				if ((camfwdvsufrint[0][i]!=null)&&(ppint[0][i]!=null)&&(camfwdvsufrint[0][i].isFinite())&&(ppint[0][i].isFinite())) {
					...
					Direction[] refcamfwdhalfvendposdir = vectorFromPoints(camposrefraction, refcamfwdhalfvendpos);
					double[] camrefcamvangle = vectorAngle(camfwddir, refcamfwdhalfvendposdir);
					double[][] refcamposcamupplanedist = planePointDistance(camposrefraction, camupplane);
					double refvmult = refcamposcamupplanedist[0][0]>=0.0f?1.0f:-1.0f;
					Matrix refractionmat = rotationMatrixAroundAxis(camdirs[1], refvmult*camrefcamvangle[0]);
